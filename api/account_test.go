...
func TestListAccountAPI(t *testing.T) {
	var n int = 5
	var accounts = make([]db.Account, n)
	for i := range n {
...
func TestAccountBalanceAPI(t *testing.T) {
	account := randomAccount()
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			q := request.URL.Query()
			q.Add("id", fmt.Sprintf("%d", tc.requestForm.ID))
			q.Add("balance", fmt.Sprintf("%d", tc.requestForm.Balance))
			request.URL.RawQuery = q.Encode()
...
// requireBodyMatchAccounts checks wheter the response body matches or not
